...
class KINDCrawler:
    ...
    def variable_setting(self):
        ...
        # 엑셀에서 5000개만 담을 수 있어서 100일 단위로 조회하여 데이터를 불러옴 (100일 정도 부르면 2900개 정도 데이터)
    ...
    def insert_to(self, file_name, table_name):
        ...
        # kind 검색(윈도우 사이즈
        # 검색 버튼을 누르려고 해서 만드는 객체, 이 때는 find_by Xpath 이므로.. Xpath 로 찾는다
        # xpath : // *[ @ id = "searchForm"] / section / div / div[3] / a[1]
        ...
        df = pd.read_html(
            ...
            header=0,  # header=0 : excel 파일의 맨 위 행을 header 로 하겠다는 뜻
            converters={'종목코드': str}  # '종목코드' 가 원래는 int 인데 str 로 받겠다. 앞자리에 0 오는것 때문
        )[0]
        ...
        # 만약에 df(데이터프레임)에 '해제일'이라는 컬럼이 있는 경우(투자경고, 투자위험 종목 / 주의종목은 없다!!)
        if '해제일' in df.columns:
            # 해제일이 아직 지정되지 않은 경우는 '-' 값이 들어있다. 이럴 때는 해제일 컬럼을 None 으로 변경
    ...
    def crawl_and_insert(self, file_name, table_name):
        ...
        search_bar = self.driver.find_element_by_css_selector('#AKCKwd')  # '#AKCKwd' : 회사명 누르는 부위의 id 값, 검사 탭에서 copy -> selector 누르면 됨
        self.driver.execute_script('arguments[0].scrollIntoView(true);', search_bar) # 추가 코드. 클릭 할 위치로 scroll
        search_bar.click()  # serach_bar 를 클릭

        # 종목 탭의 selector : #menuType > a > span 이런 식
        # a? a 는 html 의 a link, a link 안의 title = {}인 것에 나는 가겠다.
    ...
    def craw(self):
        ...
        try:
            self.clean_database()  # 투자 경고, 위험종목 table 을 삭제하는 함수 -> 왜? 위에 있다
        ...
        self.dialog_block_wait() # 로딩 끝날 때까지 대기하는 함수
       ...
       # 투자주의 / 투자경고 / 투자위험종목 리스트
        ...
        for names in insert_table_names:
            ...
            # *names : names tuple 에 해당하는 모든 요소를 parameter 로 보내줌
