...
class collector_api():
    ...
    def get_code_list(self):
        ...
        if cf.use_etf:
            stock_data['etf'] = self.remove_code_included_char(
                DataFrame([(c, '') for c in self._get_code_list_by_market(8) if c],
    ...
    def set_daily_crawler_table(self, code, code_name):
        ...
        if self.open_api.engine_daily_craw.dialect.has_table(self.open_api.engine_daily_craw, code_name):
            ...
            if dbl_dates:
                if dbl_dates[0][0] > oldest_row[
                    'date']:  # daily_buy_list 의 날짜 테이블 중 가장 과거의 날짜테이블이 API로 부터 받는 oldest_row 보다 더 최근 날짜이면
                ...
                if dc_item:
                    ...
                    if self.open_api.engine_daily_buy_list.dialect.has_table(self.open_api.engine_daily_buy_list,
                                                                             dc_date):
