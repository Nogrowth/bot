...
print("\n*************************************ex2*************************************")
# for문에 활용 -> for문의 돌아가는 변수는 key만 들어간다.
for key in company:
    val = company[key]
...
print("\n*************************************ex4*************************************")
# DataFrame ? => 행렬형태로 저장! dictionary를 이용하는게 편리함
...
df_company = DataFrame(company) #Dataframe 활용

print(f"type(df_company): {type(df_company)} ")
print(f"len: {len(df_company)}") # len = 행의 갯수
...
'''
# 아래는 0행 0열과 0행 1열을 출력, i번째의 location이라고 기억하자
...
print("\n*************************************ex6*************************************")
'''
loc:  iloc와 동일하게 DataFrame에 행, 열로 접근하는데 열의 경우 라벨명으로 접근. 직관적이다.
'''
...
# 위와 같이 코드가 길어지면 아래 처럼 수정 가능 (작은 따옴표 3개로!)
