...
class Stock:  # Stock 이라는 이름을 가진 클래스를 정의
    def __init__(self, stock_name, stock_price, stock_rate):  # __init__: '생성자'(특별한 함수) 라고 한다. 클래스의 객체를 만들 때 자동으로 실행이 된다.
        # 매직 메서드라고도 하며, class 호출되면 자동으로 실행!! 디버깅에서도 안거치고 바로 들어간다.
...
# 출력 결과 self의 일련번호와 동일!! 단, item1.name, price 등의 id는 다르다..
...
즉, 객체는 클래스의 정의대로 변수와 메서드!!!의 묶음으로 이루어져있다.
...
# 클래스 내부의 메서드를 호출할 때에도 항상 instance명으로 호출한다!!
...
class Ex6:
    def __init__(self): # 생성자의 self parameter 빼고 필수는 아니다!
        # self.x, self.y  : instance 변수
        self.x, self.y = 1, 2
